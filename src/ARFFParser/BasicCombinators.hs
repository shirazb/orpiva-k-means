...
newtype Parser t a
  = Parser { parse :: StateT [t] (StateT Position (MaybeT (Either Error))) a }
  deriving ( Monad
           , Applicative
           , Functor
           , MonadState [t]
...
runParser :: Parser t a -> [t] -> Either Error (Maybe((a,[t]), Position))
...
-- POST: Parses single case-insensitive letter
caselessChar :: Char -> Parser Char Char
caselessChar c
  | isLetter c = char (toLower c) <|> char (toUpper c)
...
-- POST: Parses case-insensitive string
caselessString :: String -> Parser Char String
caselessString ""
  = return ""
caselessString (x:xs) = do
  caselessChar x
  caselessString xs
  return (x : xs)
